...
const getDashboardStats = async (req, res) => {
  try {
    // Get total counts
    const totalUsers = await User.countDocuments({ isActive: true });
    const totalSelections = await NumberSelection.countDocuments();
    ...
    const todaySelections = await NumberSelection.countDocuments({
      createdAt: { $gte: today, $lt: tomorrow }
    });
    ...
    const activeRound = await Result.findOne({ status: 'active' });

    // Get total wallet balances
    const users = await User.find({ isActive: true }).select('wallet walletBalance');
    const totalWalletBalance = users.reduce((sum, user) => {
      return sum + (user.walletBalance || user.wallet?.balance || 0);
    }, 0);
    ...
    const recentSelections = await NumberSelection.find()
      ...
      .sort({ createdAt: -1 })
      .limit(10)
      .select('user number classType amount createdAt status');
    ...
    res.json({
      ...
      data: {
        ...
        today: {
          newUsers: todayUsers,
          newSelections: todaySelections,
          newTransactions: todayTransactions
        },
        ...
        recentActivity: recentSelections.map(selection => ({
          user: selection.user?.username || 'Unknown',
          number: selection.number,
